...
export default {
  ...
  methods: {
    ...
    initClouds() {
      ...
      for (let i = 0; i < this.cloudCount; i++) {
        ...
        this.clouds.push({
          ...
          alpha: 0,
          alphaTarget: 0.3 + Math.random() * 0.4, // 调低云朵最大透明度，让云更轻盈
    ...
    animate() {
      ...
      grad.addColorStop(0, "#87CEEB"); // 天空蓝浅色顶部
      grad.addColorStop(1, "#4682B4"); // 稍深蓝底部
      ctx.setFillStyle(grad);
      ctx.fillRect(0, 0, this.canvasWidth, this.canvasHeight);

      this.clouds.forEach((cloud) => {
        cloud.y -= cloud.speed;
        cloud.x += Math.sin(cloud.phase) * 0.3;
        cloud.phase += 0.02;
        ...
        if (cloud.y + cloud.baseSize * 0.5 < this.canvasHeight / 2) {
          cloud.alphaTarget = 0;
        }
        ...
        if (cloud.alpha === 0) {
          ...
          cloud.y = this.canvasHeight + cloud.baseSize + Math.random() * 100;
